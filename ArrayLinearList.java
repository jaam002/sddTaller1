...
public class ArrayLinearList<T> implements LinearList<T>, Iterable<T>
{
   ...
   /*public void quicksort(T[] items, int inicio, int fin ){
	   int i = inicio;
		int f = fin;
		T pivote = items[inicio];
		do{
			while(((String) items[i]).compareTo((String) pivote) > 0){
				i++;
				}
			while(((String) items[f]).compareTo((String) pivote) < 0){
				f--;
				}
			if( i <= f){
				T temp = items[i];
   ...
   public void quicksort(T[] items, int inicio, int fin, Comparator<T> c ){
	   int i = inicio;
		int f = fin;
		T pivote = items[inicio];
		do{
			while(c.compare(items[i], pivote) > 0){
				i++;
				}
			while(c.compare(items[f], pivote) < 0){
				f--;
				}
			if( i <= f){
				T temp = items[i];
